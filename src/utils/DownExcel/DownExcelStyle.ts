...
export default class DownExcel {
  ...
  down(fileName: string, callBack?: Fn){
    ...
    for(let i = 0;i < sheets.length; i++){
      ...
      let maxLevel = this.maxLevel(header);
  ...
  setData(worksheet: any, lastChild: any, data: any[], addition: any, maxLevel: number, callBack?: Fn){
    const {
      border
    } = addition;
  ...
  //  导出Excel
  async openDownloadDialog(workbook: any, fileName: string){
  ...
  resetMergeHeaderInfo(tableHeader: any[], maxLevel: number, outMarge: any, result:any[] = [], onces = true): any[] {
  ...
  //  获取当前下面所有子级
  //  即：表头横向跨度单元格数量
  getLastChild (arr: any[], result: any[] = []): any[]{
  ...
  //  1  ->  A
  //  27 ->  AA
  convertDSTo26BS(num:number): string{
    var code='';
    var reg = /^\d+$/g;
    if(!reg.test(`${num}`)){
        return code;
